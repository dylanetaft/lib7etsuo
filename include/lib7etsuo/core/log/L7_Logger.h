...
/**
 * @brief L7_RAISE_MSG_TRUSTED - Format error message and raise exception in one
 step
 ...
 * @...: Format arguments
 *
 * Combines L7_LOG_MSG_TRUSTED + L7_RAISE_MODULE_ERROR into single macro.
 * @brief Thread-safe: Yes (uses thread-local buffers)

 */
#define L7_RAISE_MSG_TRUSTED(module_name, exception, fmt, ...)                         \
  do {                                                                         \
